...
use super::super::{
    ExamplePair, FetchType, FetchedData, Msg, extract_relationships, fetch_with_cache,
};
...
) -> Vec<crate::api::metadata::models::FieldType> {
    let mut types: Vec<_> = fields.iter().map(|f| f.field_type.clone()).collect();
        ...
        Ok(data) => {
            ...
            match data {
                FetchedData::SourceFields(entity_name, mut fields) => {
                    ...
                    let relationships = if has_lookup_fields {
                        ...
                        tokio::task::block_in_place(|| {
                            tokio::runtime::Handle::current().block_on(async {
                                config
                                    .get_entity_metadata_cache(&source_env, &source_entity, 12)
                                    .await
                                    .ok()
                                    .flatten()
                    ...
                    if let Some(Resource::Success(meta)) =
                        state.source_metadata.get_mut(&entity_name)
                    {
                        meta.fields = fields;
                        meta.relationships = relationships;
                    } else {
                        state.source_metadata.insert(
                            entity_name,
                            Resource::Success(crate::api::EntityMetadata {
                                fields,
                                relationships,
                                ..Default::default()
                            }),
                ...
                FetchedData::TargetFields(entity_name, mut fields) => {
                    ...
                    let relationships = if has_lookup_fields {
                        ...
                        tokio::task::block_in_place(|| {
                            tokio::runtime::Handle::current().block_on(async {
                                config
                                    .get_entity_metadata_cache(&target_env, &target_entity, 12)
                                    .await
                                    .ok()
                                    .flatten()
                                    .map(|cached| cached.relationships)
                    ...
                    if let Some(Resource::Success(meta)) =
                        state.target_metadata.get_mut(&entity_name)
                    {
                        meta.fields = fields;
                        meta.relationships = relationships;
                    } else {
                        state.target_metadata.insert(
                            entity_name,
                            Resource::Success(crate::api::EntityMetadata {
                                fields,
                                relationships,
                ...
                FetchedData::ExampleData(pair_id, source_data, target_data) => {
                    ...
                    if let Some(pair) = state.examples.pairs.iter().find(|p| p.id == pair_id) {
                        log::debug!(
                            "Storing example data for pair {}: source_id={}, target_id={}",
                            pair_id,
                        ...
                        log::debug!(
                            "Source data has {} top-level fields",
                        ...
                        log::debug!(
                            "Target data has {} top-level fields",
                        ...
                        // TODO: Support multi-entity mode - for now use first entity
                        let first_source_entity = state
                            .source_entities
                            .first()
                            .map(|s| s.as_str())
                            .unwrap_or("");
                        let first_target_entity = state
                            .target_entities
                            .first()
                            .map(|s| s.as_str())
                            .unwrap_or("");
                        let source_key =
                            format!("{}:{}", first_source_entity, pair.source_record_id);
                        let target_key =
                            format!("{}:{}", first_target_entity, pair.target_record_id);

                        log::debug!(
                            "Storing with keys: source='{}', target='{}'",
                            source_key,
                            target_key
                        );
            ...
            if all_source_loaded && all_target_loaded {
                ...
                if is_multi_entity {
                    ...
                    state.available_source_types = source_metadata_map
                        .values()
                        .flat_map(|meta| collect_field_types(&meta.fields))
                        .collect();
                    state.available_target_types = target_metadata_map
                        .values()
                        .flat_map(|meta| collect_field_types(&meta.fields))
                        .collect();
                    ...
                    for (entity, metadata) in source_metadata_map {
                        ...
                        tokio::spawn(async move {
                            let config = crate::global_config();
                            if let Err(e) = config
                                .set_entity_metadata_cache(&source_env, &entity, &metadata)
                                .await
                            {
                                log::error!(
                                    "Failed to cache source metadata for {}/{}: {}",
                                    source_env,
                                    entity,
                                    e
                                );
                    ...
                    for (entity, metadata) in target_metadata_map {
                        ...
                        tokio::spawn(async move {
                            let config = crate::global_config();
                            if let Err(e) = config
                                .set_entity_metadata_cache(&target_env, &entity, &metadata)
                                .await
                            {
                                log::error!(
                                    "Failed to cache target metadata for {}/{}: {}",
                                    target_env,
                                    entity,
                                    e
                                );
...
pub fn handle_refresh(state: &mut State) -> Command<Msg> {
    ...
    for source_entity in &state.source_entities {
        state
            .source_metadata
            .insert(source_entity.clone(), Resource::Loading);
    }
    for target_entity in &state.target_entities {
        state
            .target_metadata
            .insert(target_entity.clone(), Resource::Loading);
    }
