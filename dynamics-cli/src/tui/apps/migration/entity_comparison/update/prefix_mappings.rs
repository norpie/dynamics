...
pub fn handle_list_navigate(state: &mut State, key: crossterm::event::KeyCode) -> Command<Msg> {
    state
        .prefix_mappings_list_state
        .handle_key(key, state.prefix_mappings.len(), 10);
    Command::None
}
...
pub fn handle_add_prefix_mapping(state: &mut State) -> Command<Msg> {
    ...
    state
        .prefix_mappings
        .insert(source_prefix.clone(), vec![target_prefix.clone()]);
    ...
    tokio::spawn(async move {
        ...
        for source_entity in &source_entities {
            for target_entity in &target_entities {
                ...
                {
                    log::error!(
                        "Failed to save prefix mapping for {}/{}: {}",
                        source_entity,
                        target_entity,
                        e
                    );
        ...
        if let Some((source_prefix, _)) = mappings_vec.get(selected_idx) {
            ...
            tokio::spawn(async move {
                ...
                for source_entity in &source_entities {
                    for target_entity in &target_entities {
                        if let Err(e) = config
                            .delete_prefix_mapping(source_entity, target_entity, &source_prefix)
                            .await
                        {
                            log::error!(
                                "Failed to delete prefix mapping for {}/{}: {}",
                                source_entity,
                                target_entity,
                                e
                            );
