...
pub fn handle_open_modal(state: &mut State) -> Command<Msg> {
    ...
    state.import_file_browser.set_filter(|entry| {
        entry.is_dir
            || entry.name.to_lowercase().ends_with(".cs")
            || entry.name.to_lowercase().ends_with(".csv")
    });
    ...
    if is_csv {
        ...
        Command::perform(
            async move {
                ...
                log::info!(
                    "Successfully parsed CSV from {}: {} manual, {} prefix, {} imported, {} source ignores, {} target ignores",
                    filename,
        ...
        Command::perform(
            async move {
                ...
                log::info!(
                    "Successfully parsed {} mappings from {}",
                    mappings.len(),
                    filename
                );
...
pub fn handle_csv_loaded(
    state: &mut State,
    csv_data: crate::csv_parser::CsvImportData,
    filename: String,
) -> Command<Msg> {
    ...
    log::info!(
        "CSV import merged: {} added, {} updated, {} removed",
        all_added.len(),
        all_updated.len(),
        all_removed.len()
    );
        ...
        async move {
            ...
            for (src, tgts) in field_mappings {
                ...
                for tgt in tgts {
                    ...
                    if let Err(e) = config
                        .set_field_mapping(src_entity, tgt_entity, src_field, tgt_field)
                        .await
                    {
                ...
                for tgt in tgts {
                    for source_entity in &source_entities {
                        for target_entity in &target_entities {
                            if let Err(e) = config
                                .set_prefix_mapping(source_entity, target_entity, &src, &tgt)
                                .await
                            {
                                log::error!(
                                    "Failed to save prefix mapping {}/{} {} -> {}: {}",
                                    source_entity,
                                    target_entity,
                                    src,
                                    tgt,
            ...
            if let Some(file) = import_file {
                ...
                let mut mappings_by_pair: std::collections::HashMap<
                    (String, String),
                ...
                for ((src_entity, tgt_entity), mappings) in mappings_by_pair {
                    if let Err(e) = config
                        .set_imported_mappings(&src_entity, &tgt_entity, &mappings, &file)
                        .await
                    {
                        log::error!(
                            "Failed to save imported mappings for {}/{}: {}",
                            src_entity,
                            tgt_entity,
                            e
                        );
            ...
            let mut ignored_by_pair: std::collections::HashMap<
                (String, String),
            ...
            for item in ignored_items {
                ...
                if let Some(field_part) = item.strip_prefix("fields:source:") {
                    let (item_entity, item_field) =
                        parse_qualified_name(field_part, default_source);
                    ...
                    let (item_entity, item_field) =
                        parse_qualified_name(field_part, default_target);
            ...
            for ((src_entity, tgt_entity), items) in ignored_by_pair {
                if let Err(e) = config
                    .set_ignored_items(&src_entity, &tgt_entity, &items)
                    .await
                {
                    log::error!(
                        "Failed to save ignored items for {}/{}: {}",
                        src_entity,
                        tgt_entity,
                        e
                    );
...
pub fn handle_mappings_loaded(
    ...
    log::info!(
        "Loading {} imported mappings from {}",
        mappings.len(),
        filename
    );
    ...
    let mappings_vec: HashMap<String, Vec<String>> = if is_multi_entity {
        log::info!(
            "Multi-entity mode: expanding {} mappings to all entity combinations",
            mappings.len()
        );
        let mut expanded = HashMap::new();
        ...
        log::info!(
            "Expanded {} simple mappings to {} qualified mappings",
            mappings.len(),
            expanded.len()
        );
        expanded
    ...
    log::info!(
        "Import results: {} added, {} updated, 0 removed (merging mode)",
        added.len(),
        updated.len()
    );
    ...
    Command::perform(
        async move {
            ...
            for source_entity in &source_entities {
                for target_entity in &target_entities {
                    if let Err(e) = config
                        .clear_imported_mappings(source_entity, target_entity)
                        .await
                    {
                        log::error!(
                            "Failed to clear imported mappings for {}/{}: {}",
                            source_entity,
                            target_entity,
                            e
                        );
...
pub fn handle_results_select(state: &mut State, index: usize) -> Command<Msg> {
    ...
    state
        .import_results_list
        .select_and_scroll(Some(index), line_count);
    Command::None
}
