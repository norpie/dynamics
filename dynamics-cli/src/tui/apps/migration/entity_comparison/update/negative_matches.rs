...
pub fn handle_list_navigate(state: &mut State, key: crossterm::event::KeyCode) -> Command<Msg> {
    state
        .negative_matches_list_state
        .handle_key(key, state.negative_matches.len(), 10);
    Command::None
}
    ...
    if let Some(selected_idx) = state.negative_matches_list_state.selected() {
        ...
        if let Some(source_field) = matches_vec.get(selected_idx) {
            ...
            let default_target_entity = state
                ...
                .map(|s| s.as_str())
                .unwrap_or("");
            let (source_entity_str, source_field_name) =
                parse_qualified_name(&source_field, default_source_entity);
            ...
            tokio::spawn(async move {
                ...
                for target_entity in &target_entities {
                    if let Err(e) = config
                        .delete_negative_match(&source_entity, target_entity, &source_field_name)
                        .await
                    {
                        log::error!(
                            "Failed to delete negative match for {}/{}: {}",
                            source_entity,
                            target_entity,
                            e
                        );
...
pub fn handle_add_negative_match_from_tree(state: &mut State) -> Command<Msg> {
    ...
    if let Some(selected_node) = source_tree.selected() {
        ...
        if let Some(match_info) = state.field_matches.get(&source_key) {
            ...
            let is_prefix_match = match_info
                .match_types
                .values()
            ...
            if !is_prefix_match && !is_prefix_type_mismatch {
                log::warn!(
                    "Cannot add negative match: field '{}' is not prefix-matched",
                    source_key
                );
                return Command::None;
            }
            ...
            tokio::spawn(async move {
                ...
                for target_entity in &target_entities {
                    if let Err(e) = config
                        .add_negative_match(&source_entity, target_entity, &source_field)
                        .await
                    {
                        log::error!(
                            "Failed to add negative match for {}/{}: {}",
                            source_entity,
                            target_entity,
                            e
                        );
            ...
            log::warn!(
                "Cannot add negative match: field '{}' has no match",
                source_key
            );
        }
    } else {
