...
use crate::tui::{
    ...
    app::App,
    command::Command,
    element::{ColumnBuilder, Element, FocusId, RowBuilder},
...
#[derive(Debug, Clone)]
enum EditingState {
    TextInput { key: String, input: String },
    Select { key: String, options: Vec<String> },
}
...
impl App for SettingsApp {
            ...
            if first_namespace == "theme" {
                ...
                commands.push(Command::perform(
                    async {
                        ...
                        crate::tui::state::config::load_all_themes(&config.options, theme_names)
                            .await
                    },
                    Msg::ThemesLoaded,
                ));
    ...
    fn update(state: &mut State, msg: Msg) -> Command<Msg> {
        match msg {
                    ...
                    if namespace == "theme" {
                        ...
                        return Command::perform(
                            async {
                                ...
                                let registry = crate::options_registry();
                                let theme_names = themes::list_themes(&registry);
                                crate::tui::state::config::load_all_themes(
                                    &config.options,
                                    theme_names,
                                )
                                .await
                        ...
                        match &opt.ty {
                            ...
                            OptionType::Enum { variants } => {
                                ...
                                let current_value = state
                                    ...
                                    .unwrap_or_default();
                                if let Some(index) =
                                    variants.iter().position(|v| v == &current_value)
                                {
                                    state.edit_select_state =
                                        crate::tui::widgets::SelectState::with_selected(index);
                                } else {
                                    state.edit_select_state =
                                        crate::tui::widgets::SelectState::new();
                                }
                            ...
                            | OptionType::String { .. } => {
                                ...
                                let current =
                                    state.values.get(&opt.key).unwrap_or(&opt.default).clone();
            ...
            Msg::NamespaceListNavigate(key) => {
                ...
                state
                    .namespace_list_state
                    .handle_key(key, state.namespaces.len(), visible_height);
            ...
            Msg::OptionListNavigate(key) => {
                let visible_height = 20;
                state.option_list_state.handle_key(
                    key,
            ...
            Msg::BoolToggled(Ok(())) => {
                ...
                Command::batch(vec![
                    ...
                    Command::perform(
                        async {
                            let new_config = crate::tui::state::RuntimeConfig::load_from_options()
                                .await
                                .map_err(|e| e.to_string())?;
                                ...
                                return Command::perform(
                                    async move {
                                        let value = match opt.ty {
                                            OptionType::UInt { .. } => {
                                                let parsed = input.parse::<u64>().map_err(|e| {
                                                    format!("Invalid number: {}", e)
                                                })?;
                                                OptionValue::UInt(parsed)
                                            }
                                            OptionType::Int { .. } => {
                                                let parsed = input.parse::<i64>().map_err(|e| {
                                                    format!("Invalid number: {}", e)
                                                })?;
                                                OptionValue::Int(parsed)
                                            }
                                            OptionType::Float { .. } => {
                                                let parsed = input.parse::<f64>().map_err(|e| {
                                                    format!("Invalid number: {}", e)
                                                })?;
                    ...
                    TextInputEvent::Changed(key_code) => {
                        if let Some(EditingState::TextInput { input, .. }) = &mut state.editing {
                            ...
                            if let Some(new_value) =
                                state.edit_input_state.handle_key(key_code, input, None)
                            {
                                *input = new_value;
                            }
                        }
            ...
            Msg::ThemesLoaded(themes) => {
                ...
                Command::batch(vec![cmd, Command::set_focus(FocusId::new("theme-select"))])
            }
            ...
            Msg::ActivateTheme(theme_name) => {
                Command::perform(
                    async move {
                        ...
                        config
                            .options
                            .set_string("theme.active", theme_name.clone())
                            .await
                            .map_err(|e| e.to_string())?;

                        // Reload runtime config
                        let new_config = RuntimeConfig::load_from_options()
                            .await
                            .map_err(|e| e.to_string())?;
                        crate::reload_runtime_config(new_config);
            ...
            Msg::CreateThemeInput(event) => {
                ...
                match event {
                    ...
                    TextInputEvent::Submit => Self::update(state, Msg::SubmitCreateTheme),
                }
            }
                ...
                Command::perform(
                    async move {
                        ...
                        for color_name in COLOR_NAMES {
                            ...
                            let value = config
                                .options
                                .get_string(&source_key)
                                .await
                                .map_err(|e| format!("Failed to read {}: {}", source_key, e))?;

                            config
                                .options
                                .set_string(&dest_key, value)
                                .await
                ...
                Command::perform(
                    async move {
                        ...
                        let hex = config
                            ...
                            .await
                            .map_err(|e| e.to_string())?;
                        let color = hex_to_color(&hex).map_err(|e| e.to_string())?;
            ...
            Msg::StartEditingColor(theme_name, color_name, color) => {
                ...
                state.color_picker_state =
                    crate::tui::widgets::ColorPickerState::from_color(color, ColorPickerMode::HSL);
            ...
            Msg::KeybindsLoaded(keybinds) => {
                ...
                let default_idx = if let Some(app) = preserved_app {
                    ...
                    state
                        .keybind_apps
                        .iter()
                        .position(|app| app == "global")
                        .unwrap_or(0)
                };

                state.selected_keybind_app_idx = default_idx;
                state.keybind_app_select_state =
                    crate::tui::widgets::SelectState::with_selected(default_idx);
            ...
            Msg::KeybindAppSelectEvent(event) => {
                ...
                // Update option count before handling event
                state
                    .keybind_app_select_state
                    .update_option_count(state.keybind_apps.len());
            ...
            Msg::SaveKeybind => {
                if let (Some(action), Some(keybind)) =
                    (&state.capturing_keybind, &state.captured_key)
                {
            ...
            Msg::ResetKeybinds => {
                Command::perform(
                    async {
                        ...
                        config
                            .options
                            .set_string("keybind.global.help", "F1".to_string())
                            .await
                            .map_err(|e| e.to_string())?;
                        config
                            .options
                            .set_string("keybind.global.app_launcher", "Ctrl+A".to_string())
                            .await
                            .map_err(|e| e.to_string())?;
                        config
                            .options
                            .set_string("keybind.global.app_overview", "Ctrl+O".to_string())
                            .await
                            .map_err(|e| e.to_string())?;
    ...
    fn view(state: &mut State) -> LayeredView<Msg> {
        ...
        let left_panel = Element::panel(namespace_list).title("Categories").build();
        ...
        let (right_panel, namespace_title) = match state.view_mode {
            ...
            ViewMode::AutoGenerated => {
                ...
                let option_list_content = if state.current_options.is_empty() {
                    ...
                    let options_with_values: Vec<OptionWithValue> = state
                        .current_options
                        .iter()
    ...
    fn subscriptions(state: &State) -> Vec<Subscription<Msg>> {
        ...
        if state.capturing_keybind.is_some() {
            ...
            for n in 1..=12 {
                let key_event =
                    crossterm::event::KeyEvent::new(KeyCode::F(n), KeyModifiers::empty());
                subs.push(Subscription::keyboard(
                    KeyCode::F(n),
                    "",
                    Msg::CaptureKey(key_event),
    ...
    fn status(_state: &State) -> Option<Line<'static>> {
        Some(Line::from(vec![Span::raw("Configure application options")]))
    }
...
impl SettingsApp {
    ...
    fn render_theme_editor(state: &mut State, theme: &crate::tui::Theme) -> (Element<Msg>, String) {
        ...
        let create_button = Element::button("theme-create", "Create New")
            .on_press(Msg::CreateTheme)
            .build();

        let delete_button = Element::button("theme-delete", "Delete")
        ...
        let panel = Element::panel(content).title("Themes").build();
    ...
    fn render_theme_preview(state: &State, runtime_theme: &crate::tui::Theme) -> Element<Msg> {
        ...
        let col1 = Element::Column {
            items: col1_items,
            spacing: 0,
        };
        let col2 = Element::Column {
            items: col2_items,
            spacing: 0,
        };
        let col3 = Element::Column {
            items: col3_items,
            spacing: 0,
        };
    ...
    fn render_keybind_editor(
        ...
        let app_select_panel = Element::panel(app_select).title("Filter by App").build();

        // Determine which app to filter by
        let filter_app = state
            .keybind_apps
            .get(state.selected_keybind_app_idx)
            .map(|s| s.as_str());
        ...
        for action in &state.keybind_actions {
            ...
            let current_keybind = state
                .keybinds
                .get(action)
                .map(|kb| kb.to_string())
                .unwrap_or_else(|| {
                    // Fall back to default from registry
                    crate::global_runtime_config()
                        .get_keybind(action)
                        .to_string()
                });
            ...
            let item = Element::button(FocusId::new(item_id), label)
        ...
        let keybind_list = if keybind_items.is_empty() {
            ...
            let col1_items: Vec<_> = keybind_items
                .iter()
                .take(per_col)
                .cloned()
                .map(|item| (Length(3), item))
                .collect();
            let col2_items: Vec<_> = keybind_items
                .iter()
                .skip(per_col)
                .take(per_col)
                .cloned()
                .map(|item| (Length(3), item))
                .collect();
            let col3_items: Vec<_> = keybind_items
                .iter()
                .skip(per_col * 2)
                .cloned()
                .map(|item| (Length(3), item))
        ...
        // Reset button
        let reset_button = Element::button("keybind-reset", "Reset to Defaults")
        ...
        let panel = Element::panel(content).title("Keybinds").build();
    ...
    fn render_color_picker_modal(state: &mut State) -> Element<Msg> {
        ...
        if let Some(editing) = &state.editing_color {
            let picker =
                Element::color_picker(FocusId::new("color-picker"), &state.color_picker_state)
                    .on_event(Msg::ColorPickerEvent)
                    .build();

            let title = format!(
                "Edit Color: {} - {}",
                editing.theme_name, editing.color_name
            );
            ...
            let preview_line = Line::from(vec![
                ...
                Span::styled(
                    colored_box,
                    Style::default().fg(state.color_picker_state.color()),
                ),
                Span::raw("  "),
            ...
            Element::panel(content).title(&title).build()
        } else {
    ...
    fn render_create_theme_modal(state: &mut State) -> Element<Msg> {
        ...
        let input_panel = Element::panel(input_widget).title("Theme Name").build();
        ...
        Element::panel(content).title("Create New Theme").build()
    }

    fn render_edit_modal(state: &mut State, opt: &OptionDefinition) -> Element<Msg> {
        let theme = &crate::global_runtime_config().theme;
        use_constraints!();

        // Build the edit widget with proper panel wrapping (matching other modals)
        let (value_panel, hint) =
            if let Some(EditingState::TextInput { key, input }) = &state.editing {
                if key == &opt.key {
                    let input_widget = Element::text_input(
